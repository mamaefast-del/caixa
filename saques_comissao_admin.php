...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['acao'])) {
    ...
    if ($acao === 'aprovar') {
        ...
        if ($stmt->execute([$id])) {
            $mensagem = "Saque recusado com sucesso!";
        }
    }
}

// Verificar se as colunas de afiliado existem
$columns_exist = false;
try {
  $pdo->query("SELECT codigo_afiliado FROM usuarios LIMIT 1");
  $columns_exist = true;
...
if ($columns_exist) {
  $stmt = $pdo->query("
    SELECT saques.*, usuarios.email, usuarios.codigo_afiliado
    FROM saques 
...
<head>
  ...
  <title>Saques de Comissão</title>
          ...
          <tbody>
            <?php foreach ($saques as $saque): ?>
              <tr>
                <td><?= $saque['id'] ?></td>
                <td>
                  <strong><?= htmlspecialchars($saque['email']) ?></strong>
                </td>
                <?php if ($columns_exist): ?>
                  <td>
                ...
                <td>
                  <?php if ($saque['status'] === 'pendente'): ?>
                    ...
                    <form action="" method="POST" style="display:inline;">
                      <input type="hidden" name="id" value="<?= $saque['id'] ?>">
                      <button type="submit" name="acao" value="recusar" class="btn btn-danger" onclick="return confirm('Deseja recusar este saque?')">
